...
3. чтобы поднять на локальной машине так же создал докер штуку для запуска с сервером кадди https://github.com/nyosru/2311test-for-candidates-docker 
4. краткая инструкция для запуска у себя, клонируете обе репы в одну папку, заходите в докер и запускаете make dev и должен запуститься докер и прописаться localhost на тест штуку
5. если будут вопросы пишите в тележку @phpcatcom подскажу как запустить или ещё чего )
